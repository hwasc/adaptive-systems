...
class HillClimberController(Controller):
    ...
    def __init__(self, left_noisemaker=None, right_noisemaker=None, gain=1):
        ...
        self.gain = gain
        self.left_gain_1 = 0.0
        self.right_gain_1 = 0.0

        self.left_gain_2 = 0.0
        self.right_gain_2 = 0.0
    ...
    def step(self, inputs, dt):
        if len(self.inputs) > 3:
            ...
            scores = [np.sum(x) for x in self.inputs[-3:]]
            
            prev_score = np.sum(scores[:3])
            curr_score = np.sum(scores[1:3]) + np.sum(inputs)

            #All is good
            # if prev_score < curr_score:
            if fitness > self.last_fitness:
                self.left_gain_2 = self.left_gain_1
                self.right_gain_2 = self.right_gain_1
                self.max_gain = 1.0
                print("inactive")
            ...
            #All is bad
                ...
                if random.uniform(0, 1) > 0.5:
                    self.left_gain_1 = random.uniform(-1, 1)# + self.left_gain_2 
                    self.left_gain_1 = max(-self.max_gain, self.left_gain_1)
                    self.left_gain_1 = min(self.max_gain, self.left_gain_1)
                else:
                    self.right_gain_1 = random.uniform(-1, 1)# + self.right_gain_2
                    self.right_gain_1 = max(-self.max_gain, self.right_gain_1)
                    self.right_gain_1 = min(self.max_gain, self.right_gain_1)
                
                self.max_gain += 0.1


            # set left motor speed
            self.left_speed_command = self.left_gain_1 #* (self.gain * inputs[1])
            # set right motor speed
            self.right_speed_command = self.right_gain_1 #* (self.gain * inputs[0])
    ...
    def fitness(self, inputs):
        ...
        # prev_score = np.sum(scores[:5])
        # curr_score = np.sum(scores[1:5]) + np.sum(inputs)
...
def run_sim(runs=1, animate=True, disturb_times=[], left_motor_noise=0.1, right_motor_noise=0.1, noise_type='brown'):
    ...
    for i in range(runs):
        ...
        controller = HillClimberController(gain=2)
