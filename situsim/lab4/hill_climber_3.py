...
class HillClimberController(Controller):
    ...
    def __init__(self, left_noisemaker=None, right_noisemaker=None, gain=1):
        ...
        self.gain = 1#gain
        self.weights_1 = [np.zeros((2, 2)), np.zeros((2, 2))]#, np.random.randn(2, 2)]
        self.biases_1 = [np.zeros(2), np.zeros(2)]
        self.weights_2 = copy.deepcopy(self.weights_1)
        self.biases_2 = copy.deepcopy(self.biases_1)

        self.last_fitness = -99999999.0
        self.weights_history = self.weights_1
    ...
    def step(self, inputs, dt):
        if len(self.inputs) > 2:
            ...
            prev_score = np.sum(self.inputs[-2])
            curr_score = np.sum(self.inputs[-1])
            ...
            #All is bad
                ...
                print("bad")
                self.weights_1 = copy.deepcopy(self.weights_2)
                self.weights_1 = self.mutate(self.weights_1)

                self.biases_1 = copy.deepcopy(self.biases_2)
                self.biases_1 = self.mutate(self.biases_1)
            
            output = np.subtract(self.inputs[-2], self.inputs[-1])

            for w,b  in zip(self.weights_1, self.biases_1):
                output = np.dot(output, w)
                # print(w)
                # print(output)
                # print(b)
                output = np.tanh(output + b)

            # set left motor speed
            self.left_speed_command = self.gain * output[0] * inputs[0]
            # set right motor speed
            self.right_speed_command = self.gain * output[1] * inputs[1]
        self.weights_history.append(self.weights_1[0])
        # print(self.inputs[-1])
