...
class NeatController(Controller):
    ...
    def step(self, inputs, dt):
        input_left = self.inputs[-1][0]
        input_right = self.inputs[-1][1]

        prev_output_left = self.left_speed_commands[-1]
        prev_output_right = self.right_speed_commands[-1]

        input = [input_left, input_right, prev_output_left, prev_output_right]
        output = self.network.activate(input)

        # set left motor speed
        self.left_speed_command = output[0]
        # set right motor speed
        self.right_speed_command = output[1]
...
def init_robot(network, light_sources):
    ...
    x = 10#random.choice(allowed_values)
    y = 10#random.choice(allowed_values)
...
if __name__ == '__main__':
    ...
    config_path = os.path.join(local_dir, 'config_ff')
    run(config_path)